...
import type { CopyState } from "@/lib/portfolio/hooks/use-copy-to-clipboard"
import { useCopyToClipboard } from "@/lib/portfolio/hooks/use-copy-to-clipboard"
    ...
    <AnimatePresence mode="popLayout" initial={false}>
      {state === "idle" ? (
        <motion.span key="idle" {...motionIconProps}>
          {idleIcon ?? <IconCopy className="size-4" />}
        </motion.span>
      ) : state === "done" ? (
        <motion.span key="done" {...motionIconProps}>
          {doneIcon ?? <IconCheck className="size-4" strokeWidth={3} />}
        </motion.span>
      ) : state === "error" ? (
        <motion.span key="error" {...motionIconProps}>
          {errorIcon ?? <IconCircleX className="size-4" />}
        </motion.span>
      ) : null}
    </AnimatePresence>
  )
